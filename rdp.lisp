...
Please, update it use whatever regexp package is available in ~A"
  ...
  (let ((lit-terminals-regexp
         (format nil "^\\(~{~A~^\\|~}\\)"
                 (mapcar
                  (lambda (item)
                    (let ((item (regexp:regexp-quote item)))
    ...
    `(defun
       ...
       (let ((match (regexp:match *spaces*
         ...
         (cond
           ...
           ((setf match (regexp:match ',lit-terminals-regexp
                                      (scanner-source scanner)
                                      :start (scanner-position scanner)))
            (setf (scanner-position scanner)      (regexp:match-end match)
                  (scanner-current-text scanner)  (regexp:match-string (scanner-source scanner) match)
                  (scanner-current-token scanner) (scanner-current-text scanner)))
           ,@(mapcar
              (lambda (terminal)
                `((setf match (regexp:match
